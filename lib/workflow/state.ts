...
export type WorkflowAction =
  ...
  | { type: "saveTask"; task: DraftWorkTask }
...
export function workflowReducer(state: WorkflowState, action: WorkflowAction): WorkflowState {
  switch (action.type) {
    ...
    case "saveTask": {
      const tasks=state.tasks.map(task=>task.id===action.task.id?structuredClone(action.task):task);
      const errors={...state.errors};
      for(const key of Object.keys(errors))if(key.startsWith(`task-${action.task.id}-`))delete errors[key];
      return {...state,...invalidateSample(state),tasks,errors};
    }
...
export function validateDraftTask(task:DraftWorkTask,tasks:DraftWorkTask[]):FormErrors {const errors:FormErrors={};if(!task.nameEn.trim())errors[`task-${task.id}-nameEn`]="English name is required.";if(task.durationMinutes===null||!Number.isInteger(task.durationMinutes)||task.durationMinutes<=0||task.durationMinutes%5!==0)errors[`task-${task.id}-durationMinutes`]="Duration must be a positive multiple of five minutes.";if((task.activityKind??"work")==="work"){if(!task.workload)errors[`task-${task.id}-workload`]="Select a workload.";if(!task.environment)errors[`task-${task.id}-environment`]="Select an environment.";if(task.splittable===null)errors[`task-${task.id}-splittable`]="Select whether the task may be split.";}if((task.requestedStart&&!task.requestedEnd)||(!task.requestedStart&&task.requestedEnd))errors[`task-${task.id}-requestedTime`]="Enter both requested times or leave both blank.";for(const issue of validateDependencyGraph(tasks.map(candidate=>candidate.id===task.id?task:candidate))){if(issue.taskId!==task.id)continue;errors[`task-${task.id}-dependencies`]=issue.code==="CIRCULAR_DEPENDENCY"?"Remove the circular dependency before continuing.":"Select only an existing work activity as a predecessor.";}return errors;}
export function validateVerifiedPlan(plan: PlanForm,tasks:DraftWorkTask[]):FormErrors { const errors=validatePlanDetails(plan);if(!tasks.length)errors.tasks="Add at least one task before continuing.";else if(!tasks.some(task=>(task.activityKind??"work")==="work"))errors.tasks="Add at least one work activity before continuing.";for(const task of tasks)Object.assign(errors,validateDraftTask(task,tasks));for(const issue of validateDependencyGraph(tasks)){const key=`task-${issue.taskId}-dependencies`;errors[key]=issue.code==="CIRCULAR_DEPENDENCY"?"Remove the circular dependency before continuing.":"Select only an existing work activity as a predecessor.";}return errors; }

...
import { HeatShiftWordmark } from "./logo-mark";
...
export function BrandHeader({ language, plan, status, onLanguageChange }: BrandHeaderProps) {
  ...
  return (
    <header className="operations-header print:hidden">
      <a href="#main-content" className="skip-link">{language === "ar" ? "انتقل إلى المحتوى الرئيسي" : "Skip to main content"}</a>
      <div className="mx-auto flex w-full max-w-[1480px] flex-col gap-4 px-4 py-3 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        <div className="brand-lockup"><p>{language === "ar" ? "تخطيط العمليات الميدانية" : "Field operations planning"}</p><HeatShiftWordmark language={language} /></div>

...
export function ResultsPage(props: ResultsPageProps) {
  ...
  const hydration=buildHydrationPlan(plan,result); const briefings=buildBriefings(plan,conditions,result);
  ...
  const addressed=new Set(result.blocks.flatMap(b=>b.reasonCodes).filter(code=>code.includes("RESTRICTION_AVOIDED")||code.includes("WORK_REST"))).size;

...
const copy = {
  en: { title:"Safer shift generated", preliminary:"Preliminary forecast plan", verified:"Supervisor-entered TWL zone", disclaimer:"Planning aid only. This report does not guarantee safety or regulatory compliance. Verify conditions through qualified on-site procedures.", before:"Before: requested plan", after:"After: planned timeline", tasks:"Original tasks", requested:"Original requested duration", conflicts:"Original-plan conflicts", noConflicts:"No original-plan conflicts detected.", work:"Work", rest:"Recovery", restriction:"Restriction", unscheduled:"Unscheduled work", metrics:"Planning summary", scheduled:"Scheduled work", recovery:"Recovery time", restrictionTime:"Restriction time", peak:"Peak forecast temperature", hydration:"Hydration planning", addressed:"Rules applied", remaining:"Remaining critical conflicts", capacity:"This shift cannot accommodate all planned work under the selected conditions.", workers:"Affected non-acclimatized workers need reassignment or supervisor intervention.", forecastNote:"Exact recovery cycles are not based on a supervisor-entered TWL zone.", twlNote:"Applied supervisor-entered TWL zone", reason:"Why this block appears", briefing:"Supervisor briefing", briefingEn:"English briefing", briefingAr:"Arabic briefing", sources:"Official sources", noEndorsement:"Sources support the configured rules; their listing does not imply endorsement of HeatShift.", edit:"Edit plan", change:"Change conditions", recalculate:"Recalculate", startOver:"Start over", print:"Print supervisor report", calculation:"Hydration planning rates", minutes:"min" },
  ar: { title:"تم إنشاء وردية أكثر أمانًا", preliminary:"خطة توقعات أولية", verified:"نطاق TWL أدخله المشرف", disclaimer:"أداة مساعدة للتخطيط فقط. لا يضمن هذا التقرير السلامة أو الامتثال التنظيمي. تحقق من الظروف من خلال إجراءات ميدانية مؤهلة.", before:"قبل: الخطة المطلوبة", after:"بعد: الجدول المخطط", tasks:"المهام الأصلية", requested:"المدة الأصلية المطلوبة", conflicts:"تعارضات الخطة الأصلية", noConflicts:"لم يتم اكتشاف تعارضات في الخطة الأصلية.", work:"عمل", rest:"تعافٍ", restriction:"تقييد", unscheduled:"عمل غير مجدول", metrics:"ملخص التخطيط", scheduled:"العمل المجدول", recovery:"وقت التعافي", restrictionTime:"وقت التقييد", peak:"درجة حرارة التوقعات القصوى", hydration:"تخطيط الترطيب", addressed:"القواعد المطبقة", remaining:"التعارضات الحرجة المتبقية", capacity:"لا يمكن لهذه الوردية استيعاب كل العمل المخطط في ظل الظروف المحددة.", workers:"يحتاج العمال غير المتأقلمين المتأثرون إلى إعادة توزيع أو تدخل المشرف.", forecastNote:"دورات التعافي الدقيقة لا تستند إلى نطاق TWL أدخله المشرف.", twlNote:"تم تطبيق نطاق TWL الذي أدخله المشرف", reason:"سبب ظهور هذه الفترة", briefing:"إحاطة المشرف", briefingEn:"الإحاطة الإنجليزية", briefingAr:"الإحاطة العربية", sources:"المصادر الرسمية", noEndorsement:"تدعم المصادر القواعد المهيأة، ولا يعني إدراجها تأييد HeatShift.", edit:"تعديل الخطة", change:"تغيير الظروف", recalculate:"إعادة الحساب", startOver:"البدء من جديد", print:"طباعة تقرير المشرف", calculation:"معدلات تخطيط الترطيب", minutes:"دقيقة" },
} as const;
...
export function ResultsPage(props: ResultsPageProps) {
  ...
  return <div className="results-report space-y-6">
    ...
    <section aria-labelledby="metrics-title"><h3 id="metrics-title" className="text-xl font-bold">{t.metrics}</h3><div className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-4">
      <Metric label={t.scheduled} value={`${result.metrics.scheduledWorkMinutes} ${t.minutes}`}/><Metric label={t.recovery} value={`${result.metrics.restMinutes} ${t.minutes}`}/><Metric label={t.restrictionTime} value={`${result.metrics.restrictionMinutes} ${t.minutes}`}/><Metric label={t.unscheduled} value={`${result.metrics.unscheduledMinutes} ${t.minutes}`} critical={result.metrics.unscheduledMinutes>0}/><Metric label={t.peak} value={result.metrics.peakForecastTemperature===null?"—":`${result.metrics.peakForecastTemperature.toFixed(1)}°C`}/><Metric label={t.hydration} value="Per worker / hour"/><Metric label={t.addressed} value={String(addressed)}/><Metric label={t.remaining} value={String(critical.length)} critical={critical.length>0}/>

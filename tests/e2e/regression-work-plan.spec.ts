...
async function confirmClassifications(page: Page) {
  ...
  await rows.nth(5).getByRole("checkbox", { name: "After Rebar + forms" }).check();
  await rows.nth(6).getByRole("checkbox", { name: "After Concrete pour" }).check();
  await rows.nth(7).getByRole("checkbox", { name: "After Finish + curing" }).check();
}
...
test("exact supervisor plan imports, confirms, and schedules in the production workflow", async ({ page }) => {
  ...
  const directSun = page.locator('[data-block-type="work"][data-task-id="ai-task-2"], [data-block-type="work"][data-task-id="ai-task-6"], [data-block-type="work"][data-task-id="ai-task-8"]');
  ...
  expect(await page.locator(`.scheduled-block.linked[data-task-id="${excavationId}"]`).count()).toBeGreaterThan(1);
  ...
  await page.screenshot({ path: "artifacts/regression-work-plan/05-mobile.png", fullPage: true });

  await page.getByRole("button", { name: "العربية" }).click();
  await expect(page.locator("html")).toHaveAttribute("dir", "rtl");
  await expect(page.getByRole("heading", { name: "الجدول المختار لوردية أكثر أمانًا", exact: true })).toBeVisible();
  await page.screenshot({ path: "artifacts/regression-work-plan/06-arabic.png", fullPage: true });

  await page.emulateMedia({ media: "print" });
  await expect(page.locator(".print-action")).toBeHidden();
  await page.screenshot({ path: "artifacts/regression-work-plan/07-print.png", fullPage: true });
});

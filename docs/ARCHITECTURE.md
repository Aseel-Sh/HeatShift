...
4. A coordinate-based model forecast and the supervisor-entered TWL zone remain visibly separate inputs.
...
`POST /api/parse-plan`, `GET /api/locations`, and `GET /api/weather` are thin HTTP adapters. They validate request boundaries, invoke server-only services, and map internal failures to typed responses without stack traces. OpenRouter and Open-Meteo payloads are locally validated before use. The browser stores a complete `SiteLocation`; weather requests use its coordinates and timezone rather than resolving a city ID on the server. Integration setup and failure behavior are documented in `docs/INTEGRATIONS.md`.
...
`DraftWorkTask` is intentionally distinct from validated scheduler activities. Drafts can retain unknown AI fields and optional original requested times; conversion occurs only after form validation. Verified work, break, and meal activities cross the scheduler boundary, and the verification table can confirm finish-to-start predecessor links. `evaluateOriginalPlan` is a pure evaluator whose `OriginalPlanConflict[]` never contains scheduler-capacity or worker-readiness findings. Sample plan, sample forecast, live forecast, AI-extracted, manual, and supervisor-entered TWL provenance are tracked explicitly. Material sample edits invalidate sample-derived forecast, conditions, and results.
